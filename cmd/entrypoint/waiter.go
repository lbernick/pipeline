...
type FS interface {
	...
	StatErr() (os.FileInfo, error)
	Name() string
}

func (w waitFile) Stat() (os.FileInfo, error)    { return os.Stat(w.filename) }
func (w waitFile) StatErr() (os.FileInfo, error) { return os.Stat(w.filename + ".err") }
func (w waitFile) Name() string                  { return w.filename }

func (rw *realWaiter) Wait(file string, expectContent bool, breakpointOnFailure bool) error {
	if file == "" {
		return nil
	}
	return rw.wait(waitFile{filename: file}, expectContent, breakpointOnFailure)
}
...
func (rw *realWaiter) wait(fs FS, expectContent bool, breakpointOnFailure bool) error {
	for ; ; time.Sleep(rw.waitPollingInterval) {
		if info, err := fs.Stat(); err == nil {
			...
			return fmt.Errorf("waiting for %q: %w", fs.Name(), err)
		}
