...
func validateTaskSpecRequestResources(ctx context.Context, taskSpec *v1beta1.TaskSpec) error {
	if taskSpec != nil {
		for _, step := range taskSpec.Steps {
			k8sResources, err := step.Resources.ToK8sResources()
			if err != nil {
				return err
			}
			for k, request := range k8sResources.Requests {
				// First validate the limit in step
				if limit, ok := k8sResources.Limits[k]; ok {
					...
					stepTemplateResources, err := taskSpec.StepTemplate.Resources.ToK8sResources()
					if err != nil {
						return err
					}
					if limit, ok := stepTemplateResources.Limits[k]; ok {
						if (&limit).Cmp(request) == -1 {
							return fmt.Errorf("Invalid request resource value: %v must be less or equal to limit %v", request.String(), limit.String())
						}
					}
